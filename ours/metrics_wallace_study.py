...
BASE_DIRS = [
    "/nfs/projects/ptgt_predictions/wallace_dataset/ours_nocorr",
    "/nfs/projects/ptgt_predictions/wallace_dataset/ours",
    "/nfs/projects/ptgt_predictions/wallace_dataset/tafasca_baseline",
    "/nfs/projects/ptgt_predictions/wallace_dataset/bansal_baseline"
]
...
# ============================================================
# Human-reference metrics (Wallace study: human vs human)
# ============================================================

def compute_human_reference_java_metrics():
    """
    Human-reference metrics for the Java data:
    uses your earlier method_map / fixation_data pairing, then
    computes shape metrics for all human–human pairs and averages them.
    """
    fixation_data = pickle.load(open("/nfs/projects/wallace_study.pkl", "rb"))
    method_map = pickle.load(open("/nfs/projects/wallace_study_mapping_file.pkl", "rb"))
    holdoutset_list = ["P3", "P4", "P5", "P6", "P7", "P8", "P9", "P10", "P11", "P12"]
    method_to_id = {}

    for data in method_map[:]:
        path = data["path"]
        if path not in method_to_id:
            method_to_id[path] = []
        participant = data["participant"]
        session = data["session"]
        method = data["method"]
        mid = f"P{participant}-T{session}.{method}"
        method_to_id[path].append(mid)
    ...
    for holdout in holdoutset_list[:]:
        ...
        for method_id in list(method_to_id.keys())[:]:
            allids = method_to_id[method_id]

            # find holdoutset for this method
            holdoutset = []
            for id_ in allids:
                participant = id_.split("-")[0]
            ...
            # compare this holdout participant against all non-holdouts for this method
            for id_ in allids:
                participant = id_.split("-")[0]
                if participant == holdout:
                    continue
                ...
                for d in holdoutset:
                    ...
                    duration = d["duration"]
                    func = d["function"]
                    holdoutdurationdict[(token, func)] = duration
                ...
                for d in nonholdout:
                    ...
                    duration = d["duration"]
                    func = d["function"]
                    ptgtdict[(token, func)] = duration

                for (token, func) in holdoutdurationdict:
                    if (token, func) in ptgtdict:
                        dur_non = ptgtdict[(token, func)]
                        dur_hold = holdoutdurationdict[(token, func)]
                        ptgtlist.append(dur_non)
                        holdoutdurationlist.append(dur_hold)

                if len(ptgtlist) <= 1:
                    continue

                # shape metrics for this human–human pair
        ...
        if temp_score:
            ...
            avg_corr = float("nan")
        print(f"Java human-reference Pearson for {holdout}: {avg_corr:.4f}")
...
def main():
    ...
    # 2) Human-reference metrics (Java and C)
    print("\n==============================")
    print("Computing Wallace study human-reference (human vs human) metrics")
    print("==============================")
    human_java_avg = compute_human_reference_java_metrics()
    ...
    for name in desired_order:
        ...
        if name == "human_reference":
            if human_java_avg is None:
                rows.append([name, "n/a", "n/a", "n/a", "n/a"])
            else:
                rows.append([
                    name,
                    f"{human_java_avg['pearson_r']:.4f}",
                    f"{human_java_avg['spearman_rho']:.4f}",
                    f"{human_java_avg['kendall_tau']:.4f}",
                    f"{human_java_avg['cosine_similarity']:.4f}",
                ])
            continue
